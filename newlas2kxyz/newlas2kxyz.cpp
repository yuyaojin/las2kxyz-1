// las2kxyz.cpp : Defines the entry point for the console application.
//
...
void usage(bool error = false, bool wait = false)
{
	...
	fprintf(stderr,
		"NEW!!! s - for an extra Morton/Hilbert code. specify it with '-key M2/M3/H2/H3'\n");
	fprintf(stderr, "NEW!!! l - for an extra Importance/LOD value.\n");
	fprintf(stderr,
		"NEW!!! -valid - to check during the encoding the tranformed XYL is in the scope.\n");
	fprintf(stderr, "NEW!!! -deltax/deltay - for global transformation.\n");
	fprintf(stderr, "NEW!!! -scalex/scaley - for global transformation.\n");
	fprintf(stderr, "---------------------------------------------\n");
	fprintf(stderr,
...
static void output_waveform(FILE* file_out, CHAR separator_sign,
	LASwaveform13reader* laswaveform13reader)
{
	...
	if (laswaveform13reader->nbits == 8)
	{
		...
		{
			fprintf(file_out, "%c%d", separator_sign,
				laswaveform13reader->samples[i]);
		}
	}
	else if (laswaveform13reader->nbits == 16)
	{
		...
		{
			fprintf(file_out, "%c%d", separator_sign,
				((U16*)laswaveform13reader->samples)[i]);
		}
	}
	else if (laswaveform13reader->nbits == 32)
	{
		...
		{
			fprintf(file_out, "%c%d", separator_sign,
				((I32*)laswaveform13reader->samples)[i]);
		}
	}
}
...
static BOOL print_attribute(FILE* file, const LASheader* header,
	const LASpoint* point, I32 index, CHAR* printstring)
{
	...
	if (header->attributes[index].data_type == 1)
	{
		...
		if (header->attributes[index].has_scale()
			|| header->attributes[index].has_offset())
		{
			F64 temp_d = header->attributes[index].scale[0] * value
				+ header->attributes[index].offset[0];
			lidardouble2string(printstring, temp_d,
				header->attributes[index].scale[0]);
...
int main(int argc, char *argv[])
{
	...
	bool bHilbert = true; //Hilbert or Morton, the default is H
	bool b3D = true; //2D or 3D code; the default is 3D, XYL; if 2D, xy;
	CHAR* key_string = 0; ///added by gxf

	double dx, dy = 0;
	double sx, sy = 1;
	int nglobal = 0; ///if use the global offset and scale; if nglobal=4, then use global

	bool bcheck = false; //to check if the global transformation is ok for this LAS file
	...
	for (i = 1; i < argc; i++)
	{
...
#else
			fprintf(stderr,
				"WARNING: not compiled with GUI support. ignoring '-gui' ...\n");
#endif
		}
		...
		else if (strcmp(argv[i], "-sep") == 0)
		{
			...
			else if (strcmp(separator, "dot") == 0
				|| strcmp(separator, "period") == 0)
			{
				separator_sign = '.';
			}
			...
			else if (strcmp(separator, "hyphen") == 0
				|| strcmp(separator, "minus") == 0)
			{
				separator_sign = '-';
			}
		...
		else if (strcmp(argv[i], "-key") == 0) //added by gxf
		{
			i++;
			//if (key_string) free(key_string);
			key_string = argv[i]; //__strdup("txyzirndecaup");

			if (strcmp(key_string, "-M2") == 0)
			{
				bHilbert = false;
				b3D = false;
			}
			if (strcmp(key_string, "-M3") == 0)
			{
				bHilbert = false;
				b3D = true;
			}
			if (strcmp(key_string, "-H2") == 0)
			{
				bHilbert = true;
				b3D = false;
			}
			if (strcmp(key_string, "-H3") == 0)
			{
				bHilbert = true;
				b3D = true;
			}
		}
		//added by gxf, the following will be used convert double xy to integer XY
		else if (strcmp(argv[i], "-deltax") == 0) //added by gxf
		{
			i++;
			dx = atof(argv[i]);
			nglobal++;
		}
		else if (strcmp(argv[i], "-deltay") == 0) //added by gxf
		{
			i++;
			dy = atof(argv[i]);
			nglobal++;
		}
		else if (strcmp(argv[i], "-scalex") == 0) //added by gxf
		{
			i++;
			sx = atof(argv[i]);
			nglobal++;
		}
		else if (strcmp(argv[i], "-scaley") == 0) //added by gxf
		{
			i++;
			sy = atof(argv[i]);
			nglobal++;
		}
		else if (strcmp(argv[i], "-valid") == 0) //added by gxf
		{
			//i++;
			bcheck = true;
		}
		else if (strcmp(argv[i], "-header") == 0)
		{
			...
			&& (lasreadopener.get_file_name_number() == 0))
		{
	...
	while (lasreadopener.active())
	{
		...
		LASwaveform13reader* laswaveform13reader =
			lasreadopener.open_waveform13(&lasreader->header);
		...
		if (opts)
		{
			...
			if ((ptsVLR = header->get_vlr("LAStools", 2000))
				|| (ptxVLR = header->get_vlr("LAStools", 2001)))
			{
				if ((parse_string == 0)
					|| (strcmp(parse_string, "original") == 0))
				{
					if (parse_string)
						free(parse_string);
					if (ptsVLR && (ptsVLR->record_length_after_header >= 32))
					{
						...
						&& (ptxVLR->record_length_after_header >= 32))
					{
					...
					{
						fprintf(stderr,
							"WARNING: found VLR for PTS with wrong payload size of %d.\n",
							ptsVLR->record_length_after_header);
					}
					else if (ptxVLR)
					{
						fprintf(stderr,
							"WARNING: found VLR for PTX with wrong payload size of %d.\n",
			...
			{
				fprintf(stderr,
					"WARNING: found no VLR with PTS or PTX info.\n");
			}
...
#else
				fprintf(file_out, "%lld       \012",
					header->extended_number_of_point_records);
#endif
			}
			else
			{
				fprintf(file_out, "%u       \012",
					header->number_of_point_records);
			}
			if (parse_string && strcmp(parse_string, "xyz")
				&& strcmp(parse_string, "xyzi")
				&& strcmp(parse_string, "xyziRGB")
				&& strcmp(parse_string, "xyzRGB"))
			{
				fprintf(stderr,
					"WARNING: the parse string for PTS should be 'xyz', 'xyzi', 'xyziRGB', or 'xyzRGB'\n");
			}
			if (separator_sign != ' ')
			{
				fprintf(stderr,
					"WARNING: the separator for PTS should be 'space' not '%s'\n",
					separator);
			}
		}
		else if (optx)
		{
			...
			if (ptxVLR && (ptxVLR->record_length_after_header == 272))
			{
				...
				fprintf(file_out, "%g %g %g\012", ((F64*)payload)[6],
					((F64*)payload)[7], ((F64*)payload)[8]); // translation
				fprintf(file_out, "%g %g %g\012", ((F64*)payload)[9],
					((F64*)payload)[10], ((F64*)payload)[11]); // rotation_row_0
				fprintf(file_out, "%g %g %g\012", ((F64*)payload)[12],
					((F64*)payload)[13], ((F64*)payload)[14]); // rotation_row_1
				fprintf(file_out, "%g %g %g\012", ((F64*)payload)[15],
					((F64*)payload)[16], ((F64*)payload)[17]); // rotation_row_2
				fprintf(file_out, "%g %g %g %g\012", ((F64*)payload)[18],
					((F64*)payload)[19], ((F64*)payload)[20],
					((F64*)payload)[21]); // transformation_row_0
				fprintf(file_out, "%g %g %g %g\012", ((F64*)payload)[22],
					((F64*)payload)[23], ((F64*)payload)[24],
					((F64*)payload)[25]); // transformation_row_0
				fprintf(file_out, "%g %g %g %g\012", ((F64*)payload)[26],
					((F64*)payload)[27], ((F64*)payload)[28],
					((F64*)payload)[29]); // transformation_row_0
				fprintf(file_out, "%g %g %g %g\012", ((F64*)payload)[30],
					((F64*)payload)[31], ((F64*)payload)[32],
					((F64*)payload)[33]); // transformation_row_0
			}
			else
			{
				if (ptxVLR)
			...
			{
				fprintf(stderr,
					"WARNING: the parse string for PTX should be 'xyz', 'xyzi', 'xyziRGB', or 'xyzRGB'\n");
			}
			if (separator_sign != ' ')
			{
				fprintf(stderr,
					"WARNING: the separator for PTX should be 'space' not '%s'\n",
					separator);
			}
		}
		...
		{
			// output header info
			fprintf(file_out, "%c file signature:            '%.4s'\012",
				header_comment_sign, header->file_signature);
			fprintf(file_out, "%c file source ID:            %d\012",
				header_comment_sign, header->file_source_ID);
			fprintf(file_out, "%c reserved (global encoding):%d\012",
				header_comment_sign, header->global_encoding);
			fprintf(file_out,
				"%c project ID GUID data 1-4:  %d %d %d '%.8s'\012",
				header_comment_sign, header->project_ID_GUID_data_1,
				header->project_ID_GUID_data_2,
				header->project_ID_GUID_data_3,
				header->project_ID_GUID_data_4);
			fprintf(file_out, "%c version major.minor:       %d.%d\012",
				header_comment_sign, header->version_major,
				header->version_minor);
			fprintf(file_out, "%c system_identifier:         '%.32s'\012",
				header_comment_sign, header->system_identifier);
			fprintf(file_out, "%c generating_software:       '%.32s'\012",
				header_comment_sign, header->generating_software);
			fprintf(file_out, "%c file creation day/year:    %d/%d\012",
				header_comment_sign, header->file_creation_day,
				header->file_creation_year);
			fprintf(file_out, "%c header size                %d\012",
				header_comment_sign, header->header_size);
			fprintf(file_out, "%c offset to point data       %u\012",
				header_comment_sign, header->offset_to_point_data);
			fprintf(file_out, "%c number var. length records %u\012",
				header_comment_sign,
				header->number_of_variable_length_records);
			fprintf(file_out, "%c point data format          %d\012",
				header_comment_sign, header->point_data_format);
			fprintf(file_out, "%c point data record length   %d\012",
				header_comment_sign, header->point_data_record_length);
			fprintf(file_out, "%c number of point records    %u\012",
				header_comment_sign, header->number_of_point_records);
			fprintf(file_out,
				"%c number of points by return %u %u %u %u %u\012",
				header_comment_sign, header->number_of_points_by_return[0],
			...
			fprintf(file_out, "%c scale factor x y z         %g %g %g\012",
				header_comment_sign, header->x_scale_factor,
				header->y_scale_factor, header->z_scale_factor);
			fprintf(file_out, "%c offset x y z               ",
				header_comment_sign);
			...
			fprintf(file_out, "%c min x y z                  ",
				header_comment_sign);
			lidardouble2string(printstring, header->min_x,
				header->x_scale_factor);
			fprintf(file_out, "%s ", printstring);
			lidardouble2string(printstring, header->min_y,
				header->y_scale_factor);
			fprintf(file_out, "%s ", printstring);
			lidardouble2string(printstring, header->min_z,
				header->z_scale_factor);
			fprintf(file_out, "%s\012", printstring);
			fprintf(file_out, "%c max x y z                  ",
				header_comment_sign);
			lidardouble2string(printstring, header->max_x,
				header->x_scale_factor);
			fprintf(file_out, "%s ", printstring);
			lidardouble2string(printstring, header->max_y,
				header->y_scale_factor);
			fprintf(file_out, "%s ", printstring);
			lidardouble2string(printstring, header->max_z,
				header->z_scale_factor);
			fprintf(file_out, "%s\012", printstring);
			// if LAS 1.4
			if (header->version_minor >= 4)
...
#else
				fprintf(file_out,
					"%c extended number of point records    %lld\012",
					header_comment_sign,
					header->extended_number_of_point_records);
				fprintf(file_out,
					"%c extended number of points by return %lld %lld %lld %lld %lld %lld %lld %lld %lld %lld %lld %lld %lld %lld %lld\012",
					header_comment_sign,
					header->extended_number_of_points_by_return[0],
					header->extended_number_of_points_by_return[1],
					header->extended_number_of_points_by_return[2],
					header->extended_number_of_points_by_return[3],
					header->extended_number_of_points_by_return[4],
					header->extended_number_of_points_by_return[5],
					header->extended_number_of_points_by_return[6],
					header->extended_number_of_points_by_return[7],
					header->extended_number_of_points_by_return[8],
					header->extended_number_of_points_by_return[9],
					header->extended_number_of_points_by_return[10],
					header->extended_number_of_points_by_return[11],
					header->extended_number_of_points_by_return[12],
					header->extended_number_of_points_by_return[13],
		...
		while (parse_string[i])
		{
			...
			case 'V': // the waveform data
				...
				{
					fprintf(stderr,
						"WARNING: requested 'V' but no waveform data available\n");
			...
			case 'E':
				...
				{
					fprintf(stderr,
						"WARNING: requested 'E' but no '-extra' specified\n");
					parse_string[i] = 's';
				}
				break;
			case '0':
			...
			case '9':
				if ((parse_string[i] - '0')
					>= lasreader->header.number_attributes)
				{
				...
				{
					attribute_starts[(parse_string[i] - '0')] =
						lasreader->header.get_attribute_start(
						(parse_string[i] - '0'));
				}
				break;
			case 's': //added by gxf --s means space filling curve
				break; //no handling
			case 'l':
				break; //no handling
			default:
...
#else
		if (verbose)
			fprintf(stderr, "processing %lld points with '%s'.\n",
			lasreader->npoints, parse_string);
#endif
		///////////////////////////////
		//check the min max of XY after the global transformation
		if (bcheck == true && nglobal == 4)
		{
			fprintf(stderr, "tranformation factors: %f, %f, %e, %e\n", dx, dy, sx, sy);

			///after the global transformation, the min
			double xmin, ymin, xmax, ymax;
			xmin = lasreader->get_min_x();
			ymin = lasreader->get_min_y();
			xmax = lasreader->get_max_x();
			ymax = lasreader->get_max_y();
			...
			if (strcmp(key_string, "M2") == 0 || strcmp(key_string, "H2") == 0)
			{
				intmax = pow((float)2, 31); //<64/2
			}
			if (strcmp(key_string, "M3") == 0 || strcmp(key_string, "H3") == 0)
			{
				intmax = pow((float)2, 21); //64/3
			}

			long xc = (xmax - dx) / sx;
			long yc = (ymax - dy) / sy;
			if ((xmax - dx) / sx > intmax || (ymax - dy) / sy > intmax)
			{
				///error,handle left
				// close the reader
				lasreader->close();
		...
		srand(time(NULL));
		for (int iter = 1; iter <= 10; iter++) //10 levels
		{
			int nlen = pow(4.0, iter);
			int nchunks = nPts / nlen;

			for (int j = 0; j < nchunks; j++)
			{
				int nsel = rand() % nlen; //the chosen one

				dataPts[j * nlen + nsel] = iter;
			}
		}
		...
		while (lasreader->read_point())
		{
			...
			while (true)
			{
				switch (parse_string[i])
				{
				case 's': //added by gxf --s means space filling curve//create the code
					////check if use the global offset and scale
					if (nglobal == 4)
					{
						XI = (lasreader->point.get_x() - dx) / sx;
						YI = (lasreader->point.get_y() - dy) / sy;
					}
					else					//local
					{
						XI = lasreader->point.get_X();
						YI = lasreader->point.get_Y();
					}

					if (verbose) fprintf(stderr, "tranformed X & Y: %d, %d\n", XI, YI);

					/////different encoding methods, 2D or 3D; Morton or Hilbert
					if (strcmp(key_string, "M2") == 0)
					{
						code = EncodeMorton2D(XI, YI);
					}
					if (strcmp(key_string, "M3") == 0)
					{
						code = mortonEncode_magicbits(XI, YI, dataPts[i]);
					}
					if (strcmp(key_string, "H2") == 0)
					{
						pt.cord[0] = XI;
					...
					if (strcmp(key_string, "H3") == 0)
					{
						...
						ConvertHkey2Str(h2, szstr, strlen(szstr));
					}

					//fprintf(file_out, "%llu", code);
					fprintf(file_out, "%s", szstr);
					break;
				case 'l':  //create the importance value, LOD
					fprintf(file_out, "%d", dataPts[pix]);
					break;

				case 'x': // the x coordinate
					lidardouble2string(printstring, lasreader->point.get_x(),
						lasreader->header.x_scale_factor);
					fprintf(file_out, "%s", printstring);
					break;
				case 'y': // the y coordinate
					lidardouble2string(printstring, lasreader->point.get_y(),
						lasreader->header.y_scale_factor);
					fprintf(file_out, "%s", printstring);
					break;
				case 'z': // the z coordinate
					lidardouble2string(printstring, lasreader->point.get_z(),
				...
				case 'i': // the intensity
					if (opts)
						fprintf(file_out, "%d",
						-2048 + lasreader->point.get_intensity());
					else if (optx)
					{
						int len;
						len = sprintf(printstring, "%.3f",
							1.0f / 4095.0f
							* lasreader->point.get_intensity()) - 1;
						while (printstring[len] == '0')
							len--;
						if (printstring[len] != '.')
							len++;
						printstring[len] = '\0';
				...
				case 'r': // the number of the return
					...
					{
						fprintf(file_out, "%d",
							lasreader->point.get_extended_return_number());
					}
					else
					{
						fprintf(file_out, "%d",
				...
				case 'c': // the classification
					if (header->point_data_format > 5)
					{
						...
						{
							fprintf(file_out, "%d",
								lasreader->point.get_extended_classification());
						}
						else
						{
							fprintf(file_out, "%d",
				...
				case 'n': // the number of returns of given pulse
					...
					{
						fprintf(file_out, "%d",
							lasreader->point.get_extended_number_of_returns());
					}
					else
					{
						fprintf(file_out, "%d",
				...
				case 'k': // the keypoint flag
					fprintf(file_out, "%d",
						lasreader->point.get_keypoint_flag());
					break;
				case 'o': // the (extended) overlap flag
					fprintf(file_out, "%d",
						lasreader->point.get_extended_overlap_flag());
					break;
				case 'R': // the red channel of the RGB field
				...
				case ')': // the raw integer X difference to the last point
					fprintf(file_out, "%d",
						lasreader->point.get_X() - last_XYZ[0]);
					break;
				case '!': // the raw integer Y difference to the last point
					fprintf(file_out, "%d",
						lasreader->point.get_Y() - last_XYZ[1]);
					break;
				case '@': // the raw integer Z difference to the last point
					fprintf(file_out, "%d",
						lasreader->point.get_Z() - last_XYZ[2]);
					break;
				case '#': // the gps-time difference to the last point
					lidardouble2string(printstring,
						lasreader->point.gps_time - last_GPSTIME);
					fprintf(file_out, "%s", printstring);
					break;
				case '$': // the R difference to the last point
					fprintf(file_out, "%d",
						lasreader->point.rgb[0] - last_RGB[0]);
					break;
				case '%': // the G difference to the last point
					fprintf(file_out, "%d",
						lasreader->point.rgb[1] - last_RGB[1]);
					break;
				case '^': // the B difference to the last point
					fprintf(file_out, "%d",
						lasreader->point.rgb[2] - last_RGB[2]);
					break;
				case '&': // the byte-wise R difference to the last point
					fprintf(file_out, "%d%c%d",
						(lasreader->point.rgb[0] >> 8) - (last_RGB[0] >> 8),
						separator_sign,
						(lasreader->point.rgb[0] & 255)
						- (last_RGB[0] & 255));
					break;
				case '*': // the byte-wise G difference to the last point
					fprintf(file_out, "%d%c%d",
						(lasreader->point.rgb[1] >> 8) - (last_RGB[1] >> 8),
						separator_sign,
						(lasreader->point.rgb[1] & 255)
						- (last_RGB[1] & 255));
					break;
				case '(': // the byte-wise B difference to the last point
					fprintf(file_out, "%d%c%d",
						(lasreader->point.rgb[2] >> 8) - (last_RGB[2] >> 8),
						separator_sign,
						(lasreader->point.rgb[2] & 255)
						- (last_RGB[2] & 255));
					break;
				case 'w': // the wavepacket index
					fprintf(file_out, "%d",
						lasreader->point.wavepacket.getIndex());
					break;
				case 'W': // all wavepacket attributes
					fprintf(file_out, "%d%c%d%c%d%c%g%c%g%c%g%c%g",
						lasreader->point.wavepacket.getIndex(),
						separator_sign,
						(U32)lasreader->point.wavepacket.getOffset(),
						separator_sign,
						lasreader->point.wavepacket.getSize(),
						separator_sign,
						lasreader->point.wavepacket.getLocation(),
						separator_sign, lasreader->point.wavepacket.getXt(),
						separator_sign, lasreader->point.wavepacket.getYt(),
						separator_sign,
				...
				case 'V': // the waVeform
					if (laswaveform13reader
						...
						&lasreader->point))
					{
						output_waveform(file_out, separator_sign,
							laswaveform13reader);
					}
					else
					{
				...
				case 'E': // the extra string
					...
					break;
				default:
					print_attribute(file_out, &lasreader->header,
						&lasreader->point, (I32)(parse_string[i] - '0'),
						printstring);
				}
...
#else
		if (verbose)
			fprintf(stderr, "converting %lld points of '%s' took %g sec.\n",
			lasreader->p_count, lasreadopener.get_file_name(),
			taketime() - start_time);
#endif
