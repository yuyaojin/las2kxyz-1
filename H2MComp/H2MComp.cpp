// H2MComp.cpp : Defines the entry point for the console application.
//
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	char* szstr2 = (char*)malloc(nstrlen + 1); //last char for zero
	memset(szstr2, 0, nstrlen + 1);

	begin = clock();
	for (iter = 0; iter < 1000000; iter++)
	{
		mcode = mortonEncode_magicbits(pt.cord[0], pt.cord[1], pt.cord[2]);

		memset(szstr2, 0, nstrlen + 1);
		for (unsigned int i = 0; i < DIM*ORDER; i += 6) //every 6 bits
		{
			ncombine = 0;			
			ncombine = ((mcode >> (i * 6)) & mask2);
			...
			*(szstr2 + i/6) = BASE64_TABLE_E2[ncombine]; //get the BASE64 character directly, just copy it
		}
	}
